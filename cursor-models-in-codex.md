...
If the network cannot carry Cursor's HTTP/2 agent stream, configure the provider explicitly with
`cursorTransport: "http1-sse"`. This uses Cursor's HTTP/1.1 `RunSSE` response stream plus ordered
`BidiAppend` requests; it does not enable polling or automatic transport fallback.
